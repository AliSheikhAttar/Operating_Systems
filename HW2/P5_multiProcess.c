...
struct Node* dfsSearch(struct Node* root, int target) {
    ...
    int ids[2];
    int j = 0;
    while (root->children[i] != NULL && j < 2  && i < 2) {
        pid_left = fork();
        if (pid_left < 0) {
            ...
            if (found != NULL) {
                exit(1); // Exit with 1 if found
            } else {
                exit(0); // Exit with 0 if not found
            }
        } else {
            ids[j++] = pid_left;
        }
    ...
    int status;
    for (int k = 0; k < j; ++k) {
        waitpid(ids[k], &status, 0);
        if (WIFEXITED(status) && WEXITSTATUS(status)) { // If child found the target
            found = root->children[k]; // Update found node
            // break;
        }
    }
